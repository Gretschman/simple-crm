...
export default function DashboardPage() {
  ...
  // Get active tasks count (todo + in_progress)
  const activeTasks = tasks?.filter(t => t.status === 'todo' || t.status === 'in_progress').length || 0
      ...
      {contacts && contacts.length > 0 ? (
        <Card>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-medium text-neutral-900">{contacts.length}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-neutral-600">Unique Companies</span>
              <span className="text-sm font-medium text-neutral-900">{uniqueCompanies}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-neutral-600">Active Tasks</span>
              <span className="text-sm font-medium text-neutral-900">{activeTasks}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-neutral-600">Latest Contact</span>
        ...
        <Card>
          <div className="text-center py-12">
            <LayoutDashboard className="mx-auto h-16 w-16 text-neutral-400" />
            <h3 className="mt-4 text-lg font-medium text-neutral-900">Welcome to Simple CRM</h3>
